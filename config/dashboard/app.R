  ...
  navbarPage(
    ...
    tabPanel(
      ...
      sidebarPanel(
        ...
        pickerInput(
          inputId = "expPol", label = h4("Experiment"), choices = names(paretoByExperiment), selected = NA,
          multiple = TRUE,  options = list(`live-search` = TRUE, size = 5, `actions-box` = TRUE)
        ),
        br(),
        h3("Policy Filtering"),
...
server <- function(input, output, session) {
  ...
  scPols <- reactive({
    ...
    expData <- paretoByExperiment[[expPol]][["paretoFront"]]
  ...
  observeEvent(
    input$tableDownload,
    {
      fn <- paste0(getwd(), "/output/data/", expPol(), "/satisficingPolicies.csv")
      write.csv(scPols(), file = fn, row.names = FALSE)
    }
  ...
  output$filteredTable <- DT::renderDataTable(
    ...
    options = list(
      # buttons = c('csv', 'copy'),
      # dom = '<"top"Bf>t<"bottom"ip>',
      dom = '<"top"f>t<"bottom"ip>',
      # dom  = '<"top">rtB<"bottom">ifp',
      # sDom  = '<"top">rB<"bottom">ifp',
