...
sys.path.append(".")
from functions.utils import round_d, slonCalculation
...
def getStLawrenceRoutingInputs(data, t):
    ...
    x["jetty1R"] = data["jetty1R"][t]
    # x["desprairiesOut"] = data["desprairiesOut"][t]
    x["richelieuOut"] = data["richelieuFlow"][t]
    x["stfrancoisOut"] = data["stfrancoisFlow"][t]
    x["stmauriceOut"] = data["stmauriceFlow"][t]
    x["tidalInd"] = data["tidalInd"][t]
    x["carillonFlow"] = data["carillonFlow"][t]
    x["chateauguayFlow"] = data["chateauguayFlow"][t]
...
def stLawrenceRouting(ontLevel, ontFlow, x):
    ...
    # stlouisFlow = tmp["lacstlouisFlow"] / 10  # divide by 10 to get 10*cms
    stlouisFlow = tmp["lacstlouisFlow"]
    slonFlow = tmp["slonFlow"]
    desprairiesOut = tmp["dpmiFlow"]
    ...
    ptclaireLevel = round_d(
        16.57 + pow((ptclaireR * (stlouisFlow / 10) / 60.4), 0.58), 2
    )
    ...
    levels = dict()
    levels["slonFlow"] = slonFlow  # cms
    levels["stlouisFlow"] = stlouisFlow  # cms
    levels["desprairiesOut"] = desprairiesOut  # cms
